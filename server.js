...
function injectNavigation(html, target) {
  ...
  const script = `<script>(function(){const p=${JSON.stringify('/proxy?url=')};const u=v=>p+encodeURIComponent(new URL(v,document.baseURI).href);document.addEventListener('click',e=>{const a=e.target.closest('a[href]');if(!a||a.target==='_blank'||a.hasAttribute('download'))return;e.preventDefault();location.href=u(a.href)});document.addEventListener('submit',e=>{const f=e.target;if(!f.action)return;f.action=u(f.action)});})();</script>`;
